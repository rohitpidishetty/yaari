...
@csrf_exempt
def yaari_image_upload_chunk(request):
    if request.method != "POST":
    ...
    try:
        ...
        if not chunk:
            return JsonResponse({"status": 400, "message": "No chunk uploaded"}, status=400)

        file_id = request.POST.get("fileId")
        chunk_index = int(request.POST.get("chunkIndex", 0))
        total_chunks = int(request.POST.get("totalChunks", 1))
        folder = request.POST.get("folder", "YaariUploads")
        file_ext = chunk.name.split(".")[-1].lower()
        content_type = chunk.content_type or "application/octet-stream"

        
        temp_key = f"{folder}/{file_id}/chunk_{chunk_index}.{file_ext}"
        ...
        if chunk_index + 1 == total_chunks:
            final_file_name = f"{folder}/{file_id}.{file_ext}"
            ...
            for i in range(total_chunks):
                chunk_key = f"{folder}/{file_id}/chunk_{i}.{file_ext}"
                obj = s3.get_object(Bucket=settings.AWS_STORAGE_BUCKET_NAME, Key=chunk_key)
                data = obj['Body'].read()
                bucket.put_object(Key=final_file_name, Body=data, ACL='public-read', ContentType=content_type)
                s3.delete_object(Bucket=settings.AWS_STORAGE_BUCKET_NAME, Key=chunk_key)

            public_url = f"https://{settings.AWS_STORAGE_BUCKET_NAME}.s3.us-east-2.amazonaws.com/{final_file_name}"
            return JsonResponse({"status": 200, "url": public_url})

        return JsonResponse({"status": 200, "message": f"Chunk {chunk_index + 1}/{total_chunks} uploaded"})
